...
namespace VRLoad
{
    public class VR_LoadLevel : MonoBehaviour
    {
        public UnityEvent loadingFadeIn, Loading, loadingFadeOut;

        private static VR_LoadLevel _active = null;
        ...
        public static float progress => (_active != null && _active.async != null) ? _active.async.progress : 0.0f;
        public static Texture progressTexture => _active != null ? _active.renderTexture : null;

        [Tooltip("Name of the level to load")] public string levelName;

        [Tooltip("Name of internal process to launch (instead of levelName).")]
        public string internalProcessPath;

        [Tooltip("The command-line args for the internal process to launch.")]
        public string internalProcessArgs;

        [Tooltip("If true, call LoadLevelAdditiveAsync instead of LoadLevelAsync.")]
        public bool loadAdditive;

        [Tooltip("Async load causes crashes in some apps.")]
        public bool loadAsync = true;

        [Tooltip("Optional logo texture.")] public Texture loadingScreen;

        [Tooltip("Optional progress bar textures.")]
        public Texture progressBarEmpty, progressBarFull;

        [Tooltip("Sizes of overlays.")] public float
            loadingScreenWidthInMeters = 6.0f,
            progressBarWidthInMeters = 3.0f;

        [Tooltip("If specified, the loading screen will be positioned in the player's view this far away.")]
        public float loadingScreenDistance = 0.0f;

        [Tooltip("Optional overrides for where to display loading screen and progress bar overlays." +
                 "\nOtherwise defaults to using this object's transform.")]
        public Transform loadingScreenTransform, progressBarTransform;

        [Tooltip("Optional skybox override textures.")]
        public Texture front, back, left, right, top, bottom;

        [Tooltip("Colors to use when dropping to the compositor between levels if no skybox is set.")]
        public Color backgroundColor = Color.black;

        [Tooltip("If false, the background color above gets applied as the foreground color in the compositor." +
                 "\nThis does not have any effect when using a skybox instead.")]
        public bool showGrid = false;

        [Tooltip("Time to fade from current scene to the compositor and back.")]
        public float fadeOutTime = 0.5f, fadeInTime = 0.5f;

        [Tooltip("Additional time to wait after finished loading before we start fading the new scene back in." +
        ...
        [Tooltip("Time to fade loading screen in and out (also used for progress bar).")]
        public float loadingScreenFadeInTime = 1.0f, loadingScreenFadeOutTime = 0.25f;
        ...
        /// <summary> Helper function to quickly and simply load a level from script. </summary>
        ...
        {
            var loader = new GameObject("loader").AddComponent<VR_LoadLevel>();
        ...
        private void OnGUI()
        {
            ...
            if (progressBarEmpty != null && progressBarFull != null)
            {
                ...
                if (progressBarOverlayHandle != OpenVR.k_ulOverlayHandleInvalid)
                {
                    var progress = async?.progress ?? 0.0f;
        ...
        private IEnumerator LoadLevel()
        {
            ...
            if (compositor != null)
            {
                if (front != null)
                {
                    Helpers.SetSkyboxOverride(front, back, left, right, top, bottom);
        ...
        ulong GetOverlayHandle(string overlayName, Transform transform, float widthInMeters = 1.0f)
        {
            ...
            var key = Helpers.overlayKey + "." + overlayName;
            ...
            if (error == EVROverlayError.None)
            {
                ...
                if (Helpers.GetTextureType() == ETextureType.DirectX)
                {
                    var textureBounds = new VRTextureBounds_t {uMin = 0, vMin = 1, uMax = 1, vMax = 0};
                    overlay.SetOverlayTextureBounds(handle, ref textureBounds);
                }

                // Convert from world space to tracking space using the top-most camera.

                var t = new Helpers.RigidTransform(transform).ToHmdMatrix34();
